		...
		<!-- Scripts -->
			...
			<script>
				function checkEmail(){
					...
					//innerHtml(바닐라js) : 태그 사이의 html 요소 포함 , html() <-jQuery
					//jquery에서 html() - > 값 가지고 오기, html("<h1>s</h1>")->값 추가하기 
					//innerText : 태그 사이의 텍스트 , text() <-jQuery
					//<input>
					//value : input 태그에 작성된 값(사용자가 입력한(선택한) 값), val() <-jQuery
					let input = $("#input").val()	//값 가져오기
					console.log(input)
					//비동기 통신 : json {"key" : "value"} , xml <>
					$.ajax({
						url : "member/emailcheck", //요청경로
						type : "get", //http요청 메서드(get-기본, post)
						data : {'input' : input},
						success : function(res){
							//res : 서버 응답값
							console.log(res)
							if(res=='success'){
								$("#result").text('사용 할 수 있는 이메일입니다.')
							}else{ //fail
								$('#result').text('사용 할 수 없는 이메일입니다.')
							}
						},
						error : function(){
							alert("통신실패!")
